...
class Save_eFace_Window(QDialog):
    def __init__(self, parent, resting_file_name,  patientID, score_brow_rest, score_PalpebralFissure_rest, score_OralCommissure_rest, NLF_at_rest,
        ...
        self._name_of_resting_file = resting_file_name  #this variable stores the name of the 
                                                        #file, it won't be modified during 
                                                        #execution                        
                                
        self._resting_file_name = resting_file_name     #this variable stores the name of the 
                                                        #file to be displayed. It will be 
                                                        #modified during execution 
        
        filename, file_extension = os.path.splitext(self._resting_file_name) 
        ...
        self._score_brow_rest = score_brow_rest
        self._score_PalpebralFissure_rest = score_PalpebralFissure_rest
        self._score_OralCommissure_rest = score_OralCommissure_rest
        self._NLF_at_rest = NLF_at_rest
        self._BrowRaise = BrowRaise
        self._GentleEyeClosure = GentleEyeClosure
        self._FullEyeClosure = FullEyeClosure
        self._OralCommissureWithSmile = OralCommissureWithSmile
        self._LowerLipEEE = LowerLipEEE
        self._OcularSynkinesis = OcularSynkinesis
        self._StaticScore = StaticScore
        self._DynamicScore = DynamicScore
        self._SynkinesisScore = SynkinesisScore
        self._Total_Score = Total_Score
        
        self._resting_file_name = photo_name  #path + file name
        self._photo_location = photo_location
        self._ID = patientID #unique identifier
    ...
    def initUI(self):
        ...
        self._file.setText(f'{self._ID}_eFace')
    ...
    def SelectFile(self):
        ...
        if not name:
            ...
            self._resting_file_name = photo_name
            
            self._file.setText(self._resting_file_name)
    ...
    def Save(self):
        ...
        Header = ['Score Brow Rest', 'Score Palpebral Fissure Rest', 'Score Oral Commissure Rest', 'NLF at Rest',
            'Brow Raise', 'Gentle Eye Closure', 'Full Eye Closure', 'Oral Commissure With Smile', 'Lower Lip Movement',
            'Ocular Synkinesis', 'Static Score', 'Dynamic Score', 'Synkinesis Score', 'Total Score']
        
        Header.insert(0,'Patient ID')
        Header.append('Procedure')
        Header.append('Pre vs Post Procedure')
        ...
        #measurements
        eFace_score = np.array([[self._score_brow_rest, self._score_PalpebralFissure_rest, self._score_OralCommissure_rest, self._NLF_at_rest,
            self._BrowRaise, self._GentleEyeClosure, self._FullEyeClosure, self._OralCommissureWithSmile, self._LowerLipEEE,
            self._OcularSynkinesis, self._StaticScore, self._DynamicScore, self._SynkinesisScore, self._Total_Score ]],dtype=object)
        ...
        fill = UI
        fill = np.append(fill, eFace_score, axis = 1)
        ...
        if self._NewFile: #the user wants to create a new file
            ...
            df = pd.DataFrame(fill, columns = Header)
